...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # if the ghost is captured the beliefs are updated so that the
        # ghost appears in it's prison cell
            allPossible[self.getJailPosition()] = 1.0
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # prob is the probability of noisy distance for the given true distance
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # iterate over old positions of ghosts
        oldPositions = self.legalPositions
        for oldPos in oldPositions:
            # obtains the distribution over new positions given old positions of ghosts
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                # update beliefs for the key pair newPos, prob
                # where prob is the probability distribution obtained over new position given
                # the old position
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particles_located = 0
        self.particles = []
        # if all the particles has not been distributed
        # evenly across the board yet then iterate again
        while particles_located < self.numParticles:
            # distribute the particles over all the legal positions
            # available on the board
            for p in self.legalPositions:
                if particles_located < self.numParticles:
                    self.particles.append(p)
                    particles_located = particles_located + 1
                # if all the particles has been distributed then break the
                # loop
                else:
                    break
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        particles_count = self.numParticles
        allPossible = util.Counter()
        previous_belief = self.getBeliefDistribution()

        # if the ghost has been captured
        if noisyDistance == None:
            # update all particles to reflect that the ghost now appears
            # in the prison cell
            self.particles = [self.getJailPosition()] * particles_count
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # prob is the probability of noisy distance for the given true distance
                prob = emissionModel[trueDistance]
                allPossible[p] = allPossible[p] + prob * previous_belief[p]
            # when all particles receive zero weight
            if not any(allPossible.values()):
                # recreate from prior distribution
                self.initializeUniformly(gameState)
            else:
                for i in range(0, self.numParticles):
                    # generate sample from belief distribution allPossible
                    # using util.sample helper method
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        count = 0
        # iterate over oldpositions in self.particles
        for oldPos in self.particles:
            # obtain distribution over new positions for the ghost
            # based on its oldPos and Pacman's current position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # generate sample from belief distribution
            self.particles[count] = util.sample(newPosDist)
            # counter is used as an index variable
            count = count + 1
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initialize with original belief state
        distribution = util.Counter()
        for particle in self.particles:
            # update distribution with current belief state
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # shuffle to ensure even placement of particles
        # across the board
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            prob = 1.0
            # iterate over all the ghosts
            for i in range(self.numGhosts):
                # if the ghost has been captured by pacman
                if noisyDistances[i] == None:
                    # update all particles so that ghost appears in
                    # prison cell
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    # probability of noisy distance given true distance
                    prob_noisydistance = emissionModels[i][trueDistance]
                    # product of all the probabilities
                    prob = prob *  prob_noisydistance
            # update the beliefs
            allPossible[particle] = allPossible[particle] + prob
        # when all particles receive zero weight
        if not any(allPossible.values()):
            # recreate from prior distribution
            self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                # obtain distribution for a single ghost, i is used at index to refer
                # to that ghost
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),i, self.ghostAgents[i])
                # generate sample for each particle based on current distribution
