...
def enhancedFeatureExtractorDigit(datum):
    ...
    index = 0
    # separate regions in each column
    for x in range(DIGIT_DATUM_WIDTH):
        separateRegions = 0
        prevPixel=datum.getPixel(x,0)
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) != prevPixel:
                    separateRegions = separateRegions + 1
                    prevPixel=datum.getPixel(x, y)
        for k in range(10):
            if separateRegions == k:
                features[index] = 1
            else:
                features[index] = 0
        index = index + 1

    # separate regions in each row
    for y in range(DIGIT_DATUM_HEIGHT):
        separateRegions = 0
        prevPixel = datum.getPixel(0,y)
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) != prevPixel:
                    separateRegions = separateRegions + 1
                    prevPixel = datum.getPixel(x, y)
    ...
    # continuous regions in each column
    for x in range(DIGIT_DATUM_WIDTH):
        continuousRegions = 0
        for y in range(DIGIT_DATUM_HEIGHT):
            if (datum.getPixel(x, y) - datum.getPixel(x, DIGIT_DATUM_HEIGHT-1-y)) != 2:
                continuousRegions = continuousRegions + 1
        if continuousRegions > 10:
            features[index] = 1
        else:
            features[index] = 0
        index = index + 1

    # continuous regions in each row
    for y in range(DIGIT_DATUM_HEIGHT):
        continuousRegions = 0
        for x in range(DIGIT_DATUM_WIDTH):
            if (datum.getPixel(x, y) - datum.getPixel(DIGIT_DATUM_WIDTH-x-1, y)) != 2:
                continuousRegions = continuousRegions + 1
        if continuousRegions > 10:
            features[index] = 1
        else:
...
def enhancedPacmanFeatures(state, action):
    ...
    # for food agent
    minD = 99999999
    for i in range(successor.getFood().width):
        for j in range(successor.getFood().height):
            if successor.getFood()[i][j] == True:
                d = util.manhattanDistance(successor.getPacmanPosition(), (i,j))
                if minD > d:
                    minD = d
    if minD == 99999999:
        features[2] = 0
    ...
    for ghost in successor.getGhostStates():
        d = util.manhattanDistance(ghost.getPosition(),successor.getPacmanPosition())
        if d < minGD:
            minGD = d
    if minGD == 99999999:
        features[3] = 0
